...
ni::PipelineState* ni::buildComputePipelineState(ComputePipelineDesc& desc) {
    if (ID3D12RootSignature* rootSignature = buildRootSignature(desc.layout, true)) {
        ...
        if (ID3D12PipelineState* pso = ni::createComputePipelineState(psoDesc)) {
            ni::PipelineState* pipelineState = new ni::PipelineState;
...
ni::PipelineState* ni::buildGraphicsPipelineState(GraphicsPipelineDesc& desc) {
...
void ni::init(int32_t x, int32_t y, uint32_t width, uint32_t height, const char* title, bool fullscreen, bool enablePIX) {
    ...
    for (uint32_t index = 0; index < NI_BACKBUFFER_COUNT; ++index) {
        ...
        renderer.swapChain->GetBuffer(index, IID_PPV_ARGS(&renderer.backbuffers[index].resource.apiResource));
        renderer.backbuffers[index].resource.apiResource->SetName(L"gfx::backbuffer");
        renderer.backbuffers[index].resource.state = D3D12_RESOURCE_STATE_PRESENT;
    }
...
void ni::waitForCurrentFrame() {
    ...
    uint64_t fenceValue = frame.fence->GetCompletedValue();
    if (fenceValue != frame.frameWaitValue) {
...
void ni::destroy() {
    ...
    for (uint32_t index = 0; index < NI_BACKBUFFER_COUNT; ++index) {
        NI_D3D_RELEASE(renderer.backbuffers[index].resource.apiResource);
    }
...
ni::FrameData* ni::beginFrame() {
    ...
    while (frame.state != FrameState::FINISHED) {
        ni::waitForCurrentFrame();
    }

    for (uint32_t index = 0; index < frame.resourcesToDestroy.getNum(); ++index) {
        NI_D3D_RELEASE(frame.resourcesToDestroy[index]);
    }

    frame.resourcesToDestroy.reset();

    // Upload texture data
    for (uint32_t index = 0; index < frame.texturesToUpload.getNum(); ++index) {
        Texture* image = frame.texturesToUpload[index];
        ...
        frame.resourcesToDestroy.add(image->upload.apiResource);
    ...
    for (uint32_t index = 0; index < frame.buffersToUpload.getNum(); ++index) {
        Buffer* buffer = frame.buffersToUpload[index];
        ...
        frame.resourcesToDestroy.add(buffer->upload.apiResource);
        buffer->upload.apiResource = nullptr;
    }
    frame.buffersToUpload.reset();

    frame.state = FrameState::STARTED;
...
ni::Resource ni::createResource(size_t sizeInBytes, D3D12_HEAP_TYPE heapType, D3D12_RESOURCE_STATES initialState, bool initToZero, D3D12_RESOURCE_FLAGS flags) {
    D3D12_RESOURCE_DESC resourceDesc = {
        D3D12_RESOURCE_DIMENSION_BUFFER,
        D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT,
        alignSize(sizeInBytes, 256),
        1,
        1,
        1,
        DXGI_FORMAT_UNKNOWN,
...
ni::Buffer* ni::createBuffer(const wchar_t* name, size_t bufferSize, BufferType type, const void* initialData, bool initToZero, D3D12_RESOURCE_FLAGS flags) {
    ni::Buffer* buffer = createBuffer(bufferSize, type, initialData, initToZero, flags);
    buffer->resource.apiResource->SetName(name);
    return buffer;
}

ni::Buffer* ni::createBuffer(size_t bufferSize, BufferType type, const void* initialData, bool initToZero, D3D12_RESOURCE_FLAGS flags) {
    D3D12_HEAP_TYPE heapType;
    D3D12_RESOURCE_STATES initialState;
    bool createUploadBuffer = false;
    switch (type) {
    case INDEX_BUFFER:
    ...
    ni::Buffer* buffer = new ni::Buffer();
    buffer->sizeInBytes = bufferSize;
    buffer->resource = createResource(bufferSize, heapType, initialState, initToZero, flags);
    buffer->cpuData = nullptr;
    if (createUploadBuffer && initialData) {
...
uint32_t ni::calcNumUint32FromSize(size_t size) {
...
ni::Texture* ni::createTexture(const wchar_t* name, uint32_t width, uint32_t height, uint32_t depth, const void* pixels, D3D12_RESOURCE_STATES initialState, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS flags) {
    ni::Texture* texture = createTexture(width, height, depth, pixels, initialState, dxgiFormat, flags);
    texture->resource.apiResource->SetName(name);
    return texture;
}

ni::Texture* ni::createTexture(uint32_t width, uint32_t height, uint32_t depth, const void* pixels, D3D12_RESOURCE_STATES initialState, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS flags) {
    ...
    NI_D3D_ASSERT(ni::getDevice()->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_ALLOW_ALL_BUFFERS_AND_TEXTURES, &resourceDesc, initialState, clearValuePtr, IID_PPV_ARGS(&texture->resource.apiResource)), "Failed to create image resource");
    ...
    if (pixels != nullptr) {
        ...
        texture->upload = ni::createResource((size_t)bufferSize, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, false, D3D12_RESOURCE_FLAG_NONE);
...
void ni::DescriptorTable::allocRTVTex2D(ni::Resource& resource, DXGI_FORMAT format, uint32_t mipSlice, uint32_t planeSlice) {
    ...
    ni::getDevice()->CreateRenderTargetView(resource.apiResource, &rtvDesc, allocate().cpuHandle);
}
...
namespace ni {
    ...
    void FlyCamera::update() {
        ...
        float moveSpeed = 4.15f * speedScale;
