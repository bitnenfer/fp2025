...
namespace ni {
	...
	struct Array {
		...
		Array(const Array<T, TSize>& copyRef) : data(nullptr), num(0), capacity(0) {
		...
		Array(Array<T, TSize>&& moveRef) noexcept : data(nullptr), num(0), capacity(0) {
			data = moveRef.data;
			num = moveRef.num;
		...
		Array<T, TSize>& operator=(const Array<T, TSize>& copyRef) {
			if (data != nullptr) {
				clear();
			}
			if (copyRef.num > 0 && copyRef.data != nullptr) {
		...
		Array<T, TSize>& operator=(Array<T, TSize>&& moveRef) noexcept {
			data = moveRef.data;
			num = moveRef.num;
			capacity = moveRef.capacity;
			moveRef.data = nullptr;
		...
		void resize(TSize newCapacity) {
			...
			if (data != nullptr && newCapacity > capacity) {
				T* newData = (T*)malloc((size_t)newCapacity * sizeof(T));
				move(newData, data, num);
				free(data);
				data = newData;
				capacity = newCapacity;
			} else if (data == nullptr && newCapacity > 0) {
	...
	struct FixedResourceBarrierBatcher {
	...
	struct ResourceBarrierBatcher {
		...
		ResourceBarrierBatcher() = default;
		...
		Array<BarrierData, uint32_t> barriers;
		Array<D3D12_RESOURCE_BARRIER, uint32_t> nativeBarriers;
	};
	...
	struct FrameData {
		...
		uint64_t frameIndex;
		void* userData;
		Array<Texture*> texturesToUpload;
		Array<Buffer*> buffersToUpload;
		Array<ID3D12Resource*> resourcesToDestroy;
		FrameState state;
	};
	...
	Resource createResource(size_t sizeInBytes, D3D12_HEAP_TYPE heapType, D3D12_RESOURCE_STATES initialState, bool initToZero, D3D12_RESOURCE_FLAGS flags);
	Buffer* createBuffer(const wchar_t* name, size_t bufferSize, BufferType type, const void* initialData = nullptr, bool initToZero = false, D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE);
	Buffer* createBuffer(size_t bufferSize, BufferType type, const void* initialData = nullptr, bool initToZero = false, D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE);
	...
	PipelineState* buildComputePipelineState(ComputePipelineDesc& desc);
